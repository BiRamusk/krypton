...
node 
{
   ...
   stage('Checkout')
   {
       ...
       sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/documentsdock-dev-apps/${MS_NAME}:documentsdock-test-apps --dry-run -o yaml >> Orchestration/deployment-documentsdock-test.yaml'
sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/documentsdock-dev-apps/${MS_NAME}:documentsdock-dev-apps --dry-run -o yaml >> Orchestration/deployment-documentsdock-dev.yaml'
  ...
  if(env.SECURITY_TESTING == 'True')
  {
   ...
   stage('Build and Tag Image for Dev')
   {
	   ...
	   FAILED_STAGE=env.STAGE_NAME
	   sh 'oc config use-context documentsdock-devops'
	  node('docker')
    {
        ...
        container('jnlp')
        {
            ...
            sh "docker build -t ${MS_NAME}:latest ."
             sh 'docker tag ${MS_NAME}:latest ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:documentsdock-dev-apps'
           sh 'docker login ${DOCKER_REGISTRY} --username $username  --password $password'
			sh 'docker push ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:documentsdock-dev-apps'
   ...
   {   
       ...
       /*sh 'oc config use-context documentsdock-dev-apps'
       sh 'oc config view'
       sh 'oc apply -f Orchestration/deployment-documentsdock-dev.yaml -n=documentsdock-dev-apps'
       sh 'oc apply -f Orchestration/service.yaml -n=documentsdock-dev-apps' */
       
   }
	
   stage('documentsdock-test - Tagging Image for Testing')
   {
   
   
        //openshiftTag(namespace: 'documentsdock-dev-apps', srcStream: '$MS_NAME', srcTag: 'latest', destStream: '$MS_NAME', destTag: 'documentsdock-test-apps')
      ...
      node('docker')
    {
	 ...
	 {
	 sh 'docker login ${DOCKER_REGISTRY} --username $username  --password $password'
    	sh 'docker pull ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:documentsdock-dev-apps'
     	 sh 'docker tag ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:documentsdock-dev-apps ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:documentsdock-test-apps'
      	  
      	  sh 'docker push ${DOCKER_REGISTRY}/${DOCKER_REPO}/$MS_NAME:documentsdock-test-apps'
	 }
	...
	stage('documentsdock-test - Deploy Application')
	{
	
       FAILED_STAGE=env.STAGE_NAME
       sh 'chmod 777 Orchestration/test/deploy.sh'
       sh 'Orchestration/test/deploy.sh'
	}
